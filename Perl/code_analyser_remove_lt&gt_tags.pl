...
sub get_single_comments{	
	return undef if(@_<1);
	#filter the comments start with // or # with at leat one Unicode word
	push @single_lines, $& while $_[0] =~ /((\/\/|\#).*(\p{L}|\p{N})+)/g;	
	#remove the 'gt' and 'lt' words generated by tags
	remove_gt_lt_tags(@single_lines);
	# again filter the comments start with // or # with at leat one Unicode word
	foreach (@single_lines){
		push @single_lines_without_tags, $_ =~ /((\/\/|\#).*(\p{L}|\p{N})+)/g;
	}
	return @single_lines_without_tags;
}
...
sub count_instruction_lines_num{
	...
	push @single_line, $& while @_[0] =~ /^((\b(?!\d)[\w]+\b|[-\+\*\%\!\=\>\<\&\|&gt&lt])+.*|.*(\b(?!\d)[\w]+\b|[-\+\*\%\!\=\>\<\&\|&gt&lt])+|.*(\b(?!\d)[\w]+\b|[-\+\*\%\!\=\>\<\&\|&gt&lt])+.*)$/gm;
...
sub get_program_words{
	return undef if(@_<1);
	my @programe_words;
	push @programe_words, $& while @_[0] =~ /(\b(?!\d)[\w]+\b|[-\+\*\%\!\=\>\<\&\|&gt&lt]+)/g;
